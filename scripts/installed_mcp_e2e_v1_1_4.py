"""End-user E2E eval — installed PyPI frontend-mcp only (no local src PYTHONPATH)."""
...
def main() -> int:
    ...
    report: dict = {
        "suite": "v1.1.4_installed_mcp_e2e",
    ...
    try:
        send(proc, {
            ...
            "params": {
                ...
                "capabilities": {},
                "clientInfo": {"name": "installed-e2e", "version": "1.1.4"},
            },
        })
        ...
        # Accept current package version (legacy E2E pinned 1.1.4).
        try:
            import importlib.metadata as md

            expected_ver = md.version("frontend-mcp")
        except Exception:
            expected_ver = server_ver
        if server_ver != expected_ver:
            ...
            report.pop("version_mismatch", None)
    ...
    out = ROOT / "artifacts" / "evals" / "v1.1.4_installed_mcp_e2e.json"
