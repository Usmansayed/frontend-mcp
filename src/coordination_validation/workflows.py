"""Load validation workflow definitions."""
...
def validation_dir() -> Path:
    return Path(__file__).resolve().parents[2] / "coordination_layer" / "validation"


@lru_cache(maxsize=1)
def load_workflows() -> dict[str, Any]:
    if yaml is None:
        raise RuntimeError("PyYAML required for coordination validation")
