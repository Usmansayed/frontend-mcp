...
3. Before large UI: pay entire card.owed / pack.critical (Evidence Pack Loop; default band=heavy). While implement_blocked, gather evidence only. Honor card.depth — do not invent ceremony beyond finish[]. Honor card.phase (intent→flow→layout→sections→components→verify→ship). Medium+ design sessions warm HTTP prefetch (inspiration/component/creative_kit) in the background — never browser.
