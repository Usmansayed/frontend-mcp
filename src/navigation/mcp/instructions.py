...
2. Read agent_summary.card — fields: class, evidence_band, pack, phase, phase_hint, orders, implement_blocked, depth, next, next_args, owed, gate, claim_ok, claim_extra, finish, resource/spine, prefetch, inspiration_pulse, can_parallel, parallel_batch, creative_kit
2a. Prefer perception_step({session_id}) to execute card.next (Tier-0). Do not invent nearby tools. Obey card.orders as short imperatives (not optional flavor).
...
3c. LEARN + COPY (80–90%), not sprinkle: after collect/pulse, VF purpose=inspiration MUST LOOK many inspiration:* blobs. Soft mood + “a little borrowed vibe” + 1–2 refs is INVALID. Require primary_ref_ids (≥3, ≥5 on very_heavy) + borrow[{ref_id,section,idea}] naming chrome to replicate (nav/aside/main/composer density, hierarchy, type). Then IMPLEMENT as near-copies (~80–90% visual match) with only small taste tweaks — do not invent a third aesthetic. After draft: VF purpose=design with chrome_fidelity[{zone:nav|aside|main|composer, fidelity:80-95, ref_id, notes}]. Mean ≥80 / zone ≥75. Then reconcile to THIS product via design_graph + consistency_audit (tokens/spacing/type) — copy structure from refs, make it consistent with the project UI. fidelity unpaid blocks claim_ok even after verify.
