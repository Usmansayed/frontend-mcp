...
YOU are the brain. The MCP returns evidence facts plus an advisory scoreboard
(coordinator / engineering_strategy). It never executes your plan for you —
read agent_summary.coordinator / recommended_next, then you decide.
...
dashboard / landing / layout / CSS task — before planning large code):
  ...
  4. Read agent_summary.coordinator + recommended_next (and engineering_strategy /
     episode_card for detail: gate, unpaid, influence, recommended_resource)
...
Playbooks by task:
  ...
  Visual feedback              → perception_visual_feedback — LOOK/judge loop
...
- Read agent_summary.blocking before advisory; read agent_summary.coordinator / recommended_next before large UI code.
...
- Reuse scan_id from observe for diff (and optional audits).

Tool groups (tools/list _meta.group): Session, Browser, Quality, Resolver, Component, Design, Resources, Inspiration, Diagnostics, Coordinator.
