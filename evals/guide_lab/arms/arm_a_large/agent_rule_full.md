...
| `episode_portfolio.unpaid` / `episode_card.portfolio.unpaid` | Structural unpaid families you still owe — **bind** (see §2b) |
| `episode_backlog.top` | Best single ROI hint — not a tunnel; must sit inside your owed plan |

---

## 2b. Agent-brain coordination (you decide — MCP stays simple)

MCP Coordination is a **facts scoreboard** (gate, unpaid, top, confidence). You are the brain.

**Loop:**

1. Read `episode_card` (or strategy): **unpaid + gate + backlog.top**  
2. Classify task: greenfield | redesign/mockup | feature | hotfix | polish  
3. Build an **owed plan (≤3 families)** from unpaid ∩ task class  
4. Call **one** tool from that plan (prefer `backlog.top` only if it is in the plan)  
5. Re-read unpaid before locking UI direction/hierarchy/foundation  
6. Then implement → Done ladder (§6)

**Task → usually owe (when unpaid):**

| Class | Owe | Skip |
|-------|-----|------|
| Greenfield / new landing | inspiration **or** snapshot; component if foundation unpaid; observe; verify (+ ship if gated) | SEO, ritual tool spam |
| Redesign / mockup | snapshot (+ SpecDiff); observe; verify; ship if gated. Inspiration only if direction still open | Re-inspiration after Spec bound |
| Feature | observe routes; resolve_* if owners unclear; verify | Greenfield inspiration |
| Hotfix / polish | observe → fix → verify | Inspiration, foundation, ship unless structure reopened |

**Inspiration when owed:** one progressive `discover`/`collect` aiming **3–5 image refs** — stop when usable or skip with a valid reason. Do **not** ritual-run 2–3 times.

**Hard fail:** obey only `gate.next` / `recommended_evidence` while other **structural** families remain unpaid (no skip/supersede). That is tunnel vision.

**Hard fail:** soft text verify as done for redesign; claim complete while gate prohibits `claim_complete`.
...
KPI hierarchy: differentiate with type/weight/color inside **equal-width** columns. Never “fix” equal KPIs with `col-span` / flex-grow that makes one card ~35%+ and truncates siblings — Ship Council will challenge `uneven_kpi_columns`.
